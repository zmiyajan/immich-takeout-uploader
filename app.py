...
/* ===========================================================================
   Immich Takeout Uploader
   Brand tokens are Immich's own (web/src/app.css and the logo artwork):
     light  primary #4250AF   bg #FFFFFF   fg #000000
     dark   primary #ACCBFA   bg #0A0A0A   gray #212121   fg #E5E7EB
     logo   #FA2921 #FFB400 #18C249 #1E83F7 #ED79B5
   Everything else derives from the scales below. No component invents its own
   spacing, radius or type size.
   =========================================================================== */
...
:root{
  ...
  --log-bg:#f6f7f9; --log-fg:#3b4252;
  --shadow:0 1px 2px rgba(16,24,40,.05);
  --shadow-lift:0 2px 8px rgba(16,24,40,.08);
  ...
  --t-xs:11px; --t-sm:12.5px; --t-md:14px; --t-lg:16.5px; --t-xl:24px;
...
body{font-size:15px;line-height:var(--lh-body);-webkit-font-smoothing:antialiased}
/* Immich tracks Latin by .1px; any tracking breaks Arabic's cursive joins. */
html[lang="en"] body{letter-spacing:.1px}
html[lang="ar"]{letter-spacing:normal}
/* Western digits in every language, never Eastern Arabic-Indic. */
body,input,button,select{font-variant-numeric:lining-nums tabular-nums}
...
/* Latin runs inside Arabic prose must be isolated or the neutral characters
   around them (colon, dash, comma, arrows) reorder and the line reads wrong. */
bdi,code,.path,.tiny{unicode-bidi:isolate}

/* --- header -------------------------------------------------------------- */
.top{display:flex;align-items:flex-start;gap:var(--s4);margin-bottom:var(--s2)}
.logo{width:44px;height:44px;flex:none;display:block}
.titles{flex:1;min-width:0;display:flex;flex-direction:column}
h1{font-size:var(--t-xl);font-weight:600;margin:0;letter-spacing:-.2px;line-height:var(--lh-tight)}
.tag{font-family:var(--mono);font-size:10.5px;color:var(--faint);letter-spacing:.14em;
     text-transform:uppercase;direction:ltr;line-height:1.6}
html[dir=rtl] .tag{text-align:right}
.toggles{display:flex;gap:var(--s2);flex:none}
.tog{background:var(--surface);color:var(--dim);border:1px solid var(--edge);
     border-radius:var(--r-full);padding:6px 13px;font-size:var(--t-sm);cursor:pointer;
     transition:color var(--ease),border-color var(--ease);box-shadow:var(--shadow);
...
.langsel select{background:transparent;border:0;color:inherit;font-family:var(--sans);
     font-size:var(--t-sm);cursor:pointer;padding:0 2px;outline:none;width:auto}
.langsel select option{background:var(--surface);color:var(--fg)}
.lede{color:var(--dim);font-size:var(--t-md);margin:var(--s3) 0 0;max-width:64ch}
.pills{display:flex;gap:var(--s2);flex-wrap:wrap;margin:var(--s5) 0 var(--s6)}
.pill{display:inline-flex;align-items:center;gap:var(--s2);background:var(--surface);
      border:1px solid var(--edge);border-radius:var(--r-full);padding:6px var(--s4);
      font-size:var(--t-sm);color:var(--dim);box-shadow:var(--shadow);white-space:nowrap}
...
/* --- step card ----------------------------------------------------------- */
.step{background:var(--surface);border:1px solid var(--edge);border-radius:var(--r-xl);
      padding:var(--s5) var(--s6);margin-bottom:var(--s4);box-shadow:var(--shadow);
      transition:border-color var(--ease)}
.step.active{border-color:var(--primary)}
.step.done{border-color:var(--ok)}
.shead{display:flex;align-items:center;gap:var(--s3);margin-bottom:var(--s4)}
...
.shead h2{font-size:var(--t-lg);font-weight:600;margin:0;line-height:var(--lh-tight)}
...
input[type=text],input[type=password],input[type=number],select{
  ...
  border-radius:var(--r-md);color:var(--fg);font-size:var(--t-md);
...
.grp input,.grp select{background:var(--surface)}

/* --- disclosure: the info button and its panel ---------------------------- */
.i{width:18px;height:18px;border-radius:50%;border:1px solid transparent;background:var(--raised);
   color:var(--faint);font-family:var(--mono);font-size:10.5px;line-height:1;cursor:pointer;
   display:grid;place-items:center;padding:0;flex:none;
   transition:background var(--ease),color var(--ease),border-color var(--ease)}
.i:hover{background:var(--primary-soft);color:var(--primary)}
.i[aria-expanded="true"]{background:var(--primary);color:var(--on-primary)}
/* Revealed with a short fade rather than an animated height: a height
...
.help-inner::before{content:"";position:absolute;top:0;bottom:0;width:3px;
      background:var(--spectrum);inset-inline-start:0;border-start-start-radius:var(--r-md);
      border-end-start-radius:var(--r-md)}
.help-inner > :first-child{margin-top:0}
...
code{font-family:var(--mono);font-size:var(--t-sm);background:var(--raised);padding:2px 7px;
     border-radius:6px;color:var(--primary);direction:ltr;display:inline-block}
code.tiny{font-size:10.5px;padding:1px 6px;border-radius:5px;color:var(--dim)}
...
.size{font-family:var(--mono);font-size:var(--t-xs);color:var(--faint);direction:ltr;unicode-bidi:isolate}

/* --- badges --------------------------------------------------------------- */
.risk{font-size:var(--t-xs);border-radius:var(--r-full);padding:2px 9px;white-space:nowrap;
...
/* --- buttons -------------------------------------------------------------- */
button{font-family:var(--sans);font-size:var(--t-md);cursor:pointer;border-radius:var(--r-full);
       border:1px solid var(--edge);padding:9px 18px;background:var(--raised);color:var(--fg);
...
button.sm{padding:7px 14px;font-size:var(--t-sm);gap:6px}
button.icon-only{padding:var(--s2);width:34px;height:34px;justify-content:center}
...
.seg{display:inline-flex;background:var(--raised);border:1px solid var(--edge);
     border-radius:var(--r-full);padding:3px;gap:2px;margin-bottom:var(--s3)}
.segbtn{border:1px solid transparent;background:transparent;border-radius:var(--r-full);
        padding:6px 15px;font-size:var(--t-sm);color:var(--dim);gap:6px}
.segbtn:hover:not(.on){color:var(--fg);border-color:transparent}
.segbtn.on{background:var(--bg);color:var(--primary);box-shadow:var(--shadow)}

/* --- collapsible group ------------------------------------------------------ */
.grp{border:1px solid var(--edge);border-radius:var(--r-lg);margin-top:var(--s3);
     overflow:hidden;background:var(--bg)}
.grp > summary{cursor:pointer;padding:11px var(--s4);font-size:var(--t-md);font-weight:500;
     color:var(--fg);display:flex;align-items:center;gap:9px;list-style:none;user-select:none;
     transition:background var(--ease)}
.grp > summary::-webkit-details-marker{display:none}
.grp > summary::after{content:"";width:6px;height:6px;border-inline-end:1.5px solid var(--faint);
     border-block-end:1.5px solid var(--faint);transform:rotate(45deg);margin-inline-start:auto;
     transition:transform var(--ease)}
.grp[open] > summary::after{transform:rotate(-135deg)}
.grp > summary:hover{background:var(--surface)}
.grp .inner{padding:0 var(--s4) var(--s4);border-top:1px solid var(--edge)}
.grp .inner > .ctl:first-child,.grp .inner > .cols:first-child{border-top:0}

/* --- file list -------------------------------------------------------------- */
.strip{background:var(--bg);border:1px solid var(--edge);border-radius:var(--r-lg);
       overflow:hidden;margin-top:var(--s3)}
.spectrum{height:3px;background:var(--spectrum)}
.crumb{font-size:12px;color:var(--faint);padding:10px var(--s4);background:var(--surface);
       border-bottom:1px solid var(--edge);overflow-x:auto;white-space:nowrap}
.crumb bdi{font-family:var(--mono);font-size:var(--t-xs);direction:ltr}
.frames{max-height:290px;overflow-y:auto}
.frame{display:flex;align-items:center;gap:var(--s3);padding:10px var(--s4);font-size:13px;
       transition:background var(--ease)}
...
.frame .nm{flex:1;font-family:var(--mono);font-size:var(--t-sm);direction:ltr;text-align:left;
       overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}
.frame .meta{font-family:var(--mono);font-size:var(--t-xs);color:var(--faint);flex:none;direction:ltr}
.frame .mark{flex:none;width:18px;display:grid;place-items:center;color:var(--faint)}
.frame.dir .nm,.frame.dir .mark{color:var(--primary)}
.frame.ok .meta,.frame.ok .mark{color:var(--ok)}
.frame.bad{background:var(--bad-bg)}
.frame.bad .meta,.frame.bad .mark{color:var(--bad)}
.empty{padding:var(--s6);text-align:center;color:var(--faint);font-size:13px}

/* --- copyable scope chips ---------------------------------------------------- */
.perms{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:5px;margin:11px 0 0}
.perms .chip{width:100%}
.chip{justify-content:center;gap:0;padding:5px 10px;border-radius:var(--r-sm);vertical-align:-.2em;
      background:var(--raised);border:1px solid transparent;font-family:var(--mono);
      font-size:12px;color:var(--primary)}
.chip:hover{border-color:var(--primary);color:var(--primary)}
/* The icon takes no width until hover, so the label sits centred at rest. */
.chip .ci{width:0;height:14px;margin-inline-start:0;opacity:0;overflow:hidden;flex:none;
      display:grid;place-items:center;
      transition:width var(--ease),margin var(--ease),opacity var(--ease)}
...
/* --- results ----------------------------------------------------------------- */
.res{margin-top:var(--s3);border-radius:var(--r-md);padding:var(--s3) var(--s4);
     font-size:var(--t-sm);background:var(--raised);color:var(--dim);line-height:var(--lh-body)}
...
.checks{border-radius:var(--r-md);overflow:hidden;background:var(--bg)}
.chk{display:flex;align-items:center;gap:var(--s3);padding:9px var(--s4);font-size:13px}
...
.chk .w{flex:1;color:var(--dim);font-size:var(--t-sm)}
.chk .n{font-size:var(--t-xs);color:var(--faint)}
.chk.y .p{color:var(--ok)} .chk.n2 .p{color:var(--bad)} .chk.u .p{color:var(--faint)}

#log{background:var(--log-bg);border:1px solid var(--edge);border-radius:var(--r-lg);
     padding:var(--s4);height:330px;overflow-y:auto;font-family:var(--mono);font-size:var(--t-xs);
     line-height:var(--lh-body);direction:ltr;text-align:left;white-space:pre-wrap;
     word-break:break-word;color:var(--log-fg);margin-top:var(--s3)}
footer{margin-top:var(--s6);text-align:center;font-size:12px;color:var(--faint)}
...
@media (max-width:600px){
  ...
  h1{font-size:21px}
  .step{padding:var(--s4);border-radius:var(--r-lg)}
  .top{flex-wrap:wrap}
  .cols{gap:var(--s3)}
}
</style>
...
<div class="wrap">
  ...
  <section class="step active" id="s1"><form onsubmit="return false" autocomplete="off">
    <div class="shead"><span class="num" data-icon="server" data-size="20"></span><h2 data-i18n="s1"></h2></div>
        ...
        <div class="ctl ctl-field">
          <div class="ctl-body">
            ...
            <p class="hint" data-i18n="s_admin"></p>
            <input type="password" id="adminKey" data-i18n-ph="ph_admin">
            <div class="help" id="hp-adminKey"><div class="help-inner" data-i18n-html="d_admin" data-flag="--admin-api-key" data-perms="job.create,job.read"></div></div>
          </div>
        </div>
    <div class="bar" style="margin-top:16px">
      ...
      <span id="testState" class="hint" style="margin:0"></span>
    </div>
    <div id="testOut"></div>
    </form>
  </section>
  ...
  <section class="step" id="s3">
    <div class="shead"><span class="num" data-icon="tune" data-size="20"></span><h2 data-i18n="s3"></h2><span class="sub" data-i18n="s3sub"></span></div>
        <div class="ctl ctl-field">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="preset" data-i18n="l_preset"></label><button type="button" class="i" data-help="hp-preset" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_preset"></p>
            <select id="preset" onchange="applyPreset()"><option value="custom" data-i18n="p_custom"></option><option value="small" data-i18n="p_small"></option><option value="medium" data-i18n="p_medium"></option><option value="large" data-i18n="p_large"></option><option value="slow" data-i18n="p_slow"></option></select>
            <div class="help" id="hp-preset"><div class="help-inner" data-i18n-html="h_preset" data-flag="" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="syncAlbums" checked>
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="syncAlbums" data-i18n="o_albums"></label><button type="button" class="i" data-help="hp-syncAlbums" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_syncAlbums"></p>
            <div class="help" id="hp-syncAlbums"><div class="help-inner" data-i18n-html="h_syncAlbums" data-flag="--sync-albums" data-perms="album.read,album.create,albumAsset.create"></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="peopleTag" checked>
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="peopleTag" data-i18n="o_people"></label><button type="button" class="i" data-help="hp-peopleTag" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_peopleTag"></p>
            <div class="help" id="hp-peopleTag"><div class="help-inner" data-i18n-html="h_peopleTag" data-flag="--people-tag" data-perms="tag.create,tag.asset"></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="takeoutTag" checked>
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="takeoutTag" data-i18n="o_takeout"></label><button type="button" class="i" data-help="hp-takeoutTag" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_takeoutTag"></p>
            <div class="help" id="hp-takeoutTag"><div class="help-inner" data-i18n-html="h_takeoutTag" data-flag="--takeout-tag" data-perms="tag.create,tag.asset"></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="continueOnError" checked>
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="continueOnError" data-i18n="o_continue"></label><button type="button" class="i" data-help="hp-continueOnError" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_continueOnError"></p>
            <div class="help" id="hp-continueOnError"><div class="help-inner" data-i18n-html="h_continueOnError" data-flag="--on-errors=continue" data-perms=""></div></div>
          </div>
        </div>
    <details class="grp">
      ...
      <div class="inner">
        <div class="ctl">
          <input type="checkbox" id="includeArchived" checked>
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="includeArchived" data-i18n="o_archived"></label><button type="button" class="i" data-help="hp-includeArchived" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_includeArchived"></p>
            <div class="help" id="hp-includeArchived"><div class="help-inner" data-i18n-html="h_includeArchived" data-flag="--include-archived" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="includePartner" checked>
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="includePartner" data-i18n="o_partner"></label><button type="button" class="i" data-help="hp-includePartner" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_includePartner"></p>
            <div class="help" id="hp-includePartner"><div class="help-inner" data-i18n-html="h_includePartner" data-flag="--include-partner" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="includeUntitled">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="includeUntitled" data-i18n="o_untitled"></label><span class="risk caution" data-i18n="r_caution"></span><button type="button" class="i" data-help="hp-includeUntitled" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_includeUntitled"></p>
            <div class="help" id="hp-includeUntitled"><div class="help-inner" data-i18n-html="h_includeUntitled" data-flag="--include-untitled-albums" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="includeTrashed">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="includeTrashed" data-i18n="o_trashed"></label><span class="risk caution" data-i18n="r_caution"></span><button type="button" class="i" data-help="hp-includeTrashed" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_includeTrashed"></p>
            <div class="help" id="hp-includeTrashed"><div class="help-inner" data-i18n-html="h_includeTrashed" data-flag="--include-trashed" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="includeUnmatched">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="includeUnmatched" data-i18n="o_unmatched"></label><span class="risk caution" data-i18n="r_caution"></span><button type="button" class="i" data-help="hp-includeUnmatched" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_includeUnmatched"></p>
            <div class="help" id="hp-includeUnmatched"><div class="help-inner" data-i18n-html="h_includeUnmatched" data-flag="--include-unmatched" data-perms=""></div></div>
          </div>
        </div>
        <div class="cols">
          <div class="ctl ctl-field">
        ...
        <div class="ctl ctl-field">
          <div class="ctl-body">
            ...
            <p class="hint" data-i18n="s_dateRange"></p>
            <input type="text" id="dateRange" placeholder="2015-01-01,2018-12-31">
            <div class="help" id="hp-dateRange"><div class="help-inner" data-i18n-html="h_dateRange" data-flag="--date-range" data-perms=""></div></div>
          </div>
        </div>
      </div>
    </details>
      ...
      <div class="inner">
        ...
        <div class="ctl ctl-field">
          <div class="ctl-body">
            ...
            <p class="hint" data-i18n="s_burst"></p>
            <select id="manageBurst"><option value="NoStack">NoStack</option><option value="Stack">Stack</option><option value="StackKeepRaw">StackKeepRaw</option><option value="StackKeepJPEG">StackKeepJPEG</option></select>
            <div class="help" id="hp-manageBurst"><div class="help-inner" data-i18n-html="h_burst" data-flag="--manage-burst" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="epsonFastFoto">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="epsonFastFoto" data-i18n="o_epson"></label><button type="button" class="i" data-help="hp-epsonFastFoto" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_epsonFastFoto"></p>
            <div class="help" id="hp-epsonFastFoto"><div class="help-inner" data-i18n-html="h_epson" data-flag="--manage-epson-fastfoto" data-perms=""></div></div>
          </div>
        </div>
      </div>
    </details>
    <details class="grp">
      ...
      <div class="inner">
        <div class="ctl">
          <input type="checkbox" id="sessionTag">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="sessionTag" data-i18n="o_session"></label><button type="button" class="i" data-help="hp-sessionTag" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_sessionTag"></p>
            <div class="help" id="hp-sessionTag"><div class="help-inner" data-i18n-html="h_sessionTag" data-flag="--session-tag" data-perms="tag.create,tag.asset"></div></div>
          </div>
        </div>
        <div class="ctl ctl-field">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="tags" data-i18n="l_tags"></label><button type="button" class="i" data-help="hp-tags" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_tags"></p>
            <input type="text" id="tags" placeholder="migration/2015, family">
            <div class="help" id="hp-tags"><div class="help-inner" data-i18n-html="h_tags" data-flag="--tag" data-perms="tag.create,tag.asset"></div></div>
          </div>
        </div>
        <div class="ctl ctl-field">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="partnerAlbum" data-i18n="l_partneralbum"></label><button type="button" class="i" data-help="hp-partnerAlbum" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_partnerAlbum"></p>
            <input type="text" id="partnerAlbum" placeholder="Shared with me">
            <div class="help" id="hp-partnerAlbum"><div class="help-inner" data-i18n-html="h_partnerAlbum" data-flag="--partner-shared-album" data-perms=""></div></div>
          </div>
        </div>
      </div>
    </details>
      ...
      <div class="inner">
        ...
        <div class="ctl ctl-field">
          <div class="ctl-body">
            ...
            <p class="hint" data-i18n="s_timeZone"></p>
            <input type="text" id="timeZone" placeholder="Asia/Riyadh">
            <div class="help" id="hp-timeZone"><div class="help-inner" data-i18n-html="h_timeZone" data-flag="--time-zone" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="overwrite">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="overwrite" data-i18n="o_overwrite"></label><span class="risk danger" data-i18n="r_danger"></span><button type="button" class="i" data-help="hp-overwrite" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_overwrite"></p>
            <div class="help" id="hp-overwrite"><div class="help-inner" data-i18n-html="h_overwrite" data-flag="--overwrite" data-perms="asset.delete"></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="skipVerifySSL">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="skipVerifySSL" data-i18n="o_ssl"></label><span class="risk privacy" data-i18n="r_privacy"></span><button type="button" class="i" data-help="hp-skipVerifySSL" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_skipVerifySSL"></p>
            <div class="help" id="hp-skipVerifySSL"><div class="help-inner" data-i18n-html="h_skipVerifySSL" data-flag="--skip-verify-ssl" data-perms=""></div></div>
          </div>
        </div>
        <div class="ctl">
          <input type="checkbox" id="apiTrace">
          <div class="ctl-body">
            <div class="ctl-head"><label class="ctl-title" for="apiTrace" data-i18n="o_trace"></label><span class="risk privacy" data-i18n="r_privacy"></span><button type="button" class="i" data-help="hp-apiTrace" aria-expanded="false">i</button></div>
            <p class="hint" data-i18n="s_apiTrace"></p>
            <div class="help" id="hp-apiTrace"><div class="help-inner" data-i18n-html="h_apiTrace" data-flag="--api-trace" data-perms=""></div></div>
          </div>
        </div>
      </div>
    </details>
...
function test() {
  ...
  fetch("/test", {
    ...
    $("testOut").innerHTML =
      ...
      + '<div class="inner" style="padding:0"><div class="checks">'
