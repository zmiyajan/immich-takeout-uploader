...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .chip{justify-content:center;gap:6px;padding:5px 10px;border-radius:8px;vertical-align:-.2em;
        background:var(--raised);border:1px solid transparent;font-family:var(--mono);
        font-size:12px;color:var(--primary);direction:ltr;position:relative}
  .chip:hover{border-color:var(--primary);color:var(--primary)}
  .chip .ci{width:14px;height:14px;display:grid;place-items:center;opacity:0;transition:opacity .15s;flex:none}
  .chip:hover .ci,.chip.copied .ci{opacity:.75}
  .chip.copied{color:var(--ok);border-color:var(--ok)}
  .copybar{display:flex;align-items:center;gap:10px;margin-top:9px;flex-wrap:wrap}
  .copyall{padding:5px 12px;font-size:12.5px;gap:6px}
  .copyall .ci{width:14px;height:14px;display:grid;place-items:center;flex:none}
  .copyall.copied{color:var(--ok);border-color:var(--ok)}
...
<div class="wrap">
  ...
  <section class="step" id="s3">
    ...
    <div class="field" style="margin-top:16px">
      <label><span data-i18n="l_conc"></span><button type="button" class="i" data-help="h-conc" aria-expanded="false">i</button></label>
...
function paintCopy() {
  ...
  document.querySelectorAll(".copyall").forEach(function (b) {
    var slot = b.querySelector(".ci"), label = b.querySelector(".cn");
    if (slot && !slot.innerHTML) { slot.innerHTML = icon("copy", 14); }
    if (label) { label.textContent = t("copy_all"); }
  });
...
document.addEventListener("click", function (e) {
  ...
  var all = e.target.closest && e.target.closest("[data-copy-all]");
  if (all) {
    /* Only the nine scopes in the grid, not the inline tag chips beside it. */
    var names = [].slice.call(document.querySelectorAll(".perms .chip[data-copy]"))
      .map(function (c) { return c.getAttribute("data-copy"); });
    copyText(names.join("\\n"))
      .then(function () { flashCopied(all); })
      .catch(function () { all.title = "copy failed"; });
    return;
  }
